...
function createMemoryFireflies(){const scene=$('a-scene'),count=38;for(let i=0;i<count;i++){const angle=i/count*Math.PI*2,radius=2.8+(i%7)*.86,x=Math.sin(angle)*radius,z=Math.cos(angle)*radius,y=.85+(i%8)*.58,yaw=angle*180/Math.PI;const insect=make('a-image',{class:'memory-firefly',src:'#realistic-firefly',width:String(.22+(i%3)*.045),height:String(.15+(i%3)*.03),position:`${x} ${y} ${z}`,rotation:`0 ${yaw} ${-12+(i%5)*6}`,material:'shader:flat;transparent:true;alphaTest:.04;side:double',animation:`property:position;dir:alternate;loop:true;dur:${1450+i*71};easing:easeInOutSine;to:${x+.16*Math.cos(angle)} ${y+.34} ${z+.16*Math.sin(angle)}`,animation__piscar:`property:material.opacity;from:.48;to:1;dir:alternate;loop:true;dur:${520+(i%6)*130};easing:easeInOutSine`});scene.append(insect)}}

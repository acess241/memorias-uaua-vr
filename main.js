...
function enterPanorama(panorama){
  ...
  if(guide){const host=make('a-entity',{id:'visit-guide',position:'-4.8 0 -3.4',rotation:'0 55 0'});const character=make('a-plane',{width:'2.1',height:'4.2',position:'0 2.1 0',material:'shader:flat;transparent:true;alphaTest:.025;side:double'});character.setAttribute('waving-guide',{src:assetPath(guide.image)});host.append(character);host.append(make('a-plane',{width:'3.65',height:'1.28',position:'-2.85 1.58 .01',material:'shader:flat;color:#073F73;opacity:.98'}));host.append(canvasLabel(guide.name,{width:3.36,height:.46,position:'-2.85 1.85 .03',color:'#FFFFFF',fontSize:82,align:'center',weight:'700'}));host.append(canvasLabel(guide.role,{width:3.3,height:.62,position:'-2.85 1.35 .03',color:'#FDBA18',fontSize:59,align:'center',weight:'700'}));scene.append(host)}
